...
const PayrollList = () => {
  ...
  const handleApprove = async () => {
    try {
      await payrollAPI.approve(approveModal.payroll._id, {
        notes: approvalNotes,
      });
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `payroll-${filters.month}-${filters.year}.csv`
      );
      document.body.appendChild(link);
      link.click();
      link.remove();
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Manage employee payrolls and salary processing
          </p>
        </div>
      ...
      <div className="card">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="flex items-end">
            <button
              onClick={() =>
                setFilters({ month: "", year: "", status: "", employee: "" })
              }
                ...
                <tr key={payroll._id}>
                  ...
                  <td className="table-cell">
                    <div className="text-sm text-gray-900">
                      {
                        months.find((m) => m.value === payroll.payPeriod.month)
                          ?.label
                      }{" "}
                    ...
                    <div className="flex space-x-2">
                      ...
                      {hasPermission("approve_payroll") &&
                        payroll.status === "Pending" && (
                          <button
                            onClick={() =>
                              setApproveModal({ open: true, payroll })
                            }
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-600">
            Approve payroll for{" "}
            {approveModal.payroll?.employee?.personalInfo?.firstName}{" "}
            {approveModal.payroll?.employee?.personalInfo?.lastName}?
          </p>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
